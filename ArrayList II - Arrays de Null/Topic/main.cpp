...
public:
    Pass(std::string id = "", int idade = 0) : id(id), idade(idade)
    {
    }
...
class Topic
{
    ...
    {
        int pE = procurarVaga(cadeiras);

        cadeiras[pE] = std::make_shared<Pass>(pass->getId(), pass->getIdade());
    ...
    {
        int posToRemove = procurarPorNome(id, cadeiras);

        auto personRemoved = cadeiras[posToRemove];
        cadeiras[posToRemove] = nullptr;
...
public:
    ...
    std::string str() const
    {
        ...
        for (int i = 0; i < (int)preferenciais.size(); i++)
        {
            int j = preferenciais.size() - 1;

            if (preferenciais[i] == nullptr)
                ss << "@ ";
            else if (preferenciais[i] != nullptr && i == j)
        ...
        for (int i = 0; i < (int)normais.size(); i++)
        {
            int j = normais.size() - 1;

            if (normais[i] == nullptr && i == j)
                ss << "=";
            else if (normais[i] == nullptr)
                ss << "= ";
            else if (normais[i] != nullptr && i == j)
                ss << "=" << normais[i]->str();
            else
                ss << "=" << normais[i]->str() << " ";
        }
