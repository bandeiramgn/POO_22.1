...
#define FMAPE(x, fx) PIPE([&](auto x) { return fx; })
    ...
    struct FMT
    {
        ...
        std::string prefix;
        std::string suffix;
        FMT(std::string separator = ", ", std::string prefix = "[", std::string suffix = "]") : separator(separator), prefix(prefix), suffix(suffix) {}
    ...
    {
        return container | FOLD([](auto acc, auto x)
                                { return acc + x; },
                                0);
    };
